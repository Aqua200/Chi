...
*👉🏻 Puedes contactar al propietari@ del Bot si crees que se trata de un error o para charlar sobre tu desbaneo*
...
conn.sendMessage(m.chat, {text: `*⚠ sᴜs ᴅɪᴀᴍᴀɴᴛᴇ 💎 sᴇ ʜᴀɴ ᴀɢᴏᴛᴀᴅᴏ ᴘᴜᴇᴅᴇ ᴄᴏᴍᴘʀᴀʀ ᴍᴀs ᴜsᴀɴᴅᴏ ᴇʟ ᴄᴏᴍᴀɴᴅᴏ:* #buy`, contextInfo: {externalAdReply : {mediaUrl: null, mediaType: 1, description: null, "title": wm, body: '', previewType: 0, "thumbnail": img.getRandom(), sourceUrl: [nna, md, yt, nn, ig, tiktok].getRandom()}}}, { quoted: m })             
continue
}
if (plugin.level > _user.level) {
conn.sendMessage(m.chat, {text: `*⚠️ ɴᴇᴄᴇsɪᴛᴀ ᴇʟ ɴɪᴠᴇʟ ${plugin.level} ᴘᴀʀᴀ ᴘᴏᴅᴇʀ ᴜsᴀʀ ᴇsᴛᴇ ᴄᴏᴍᴀɴᴅᴏ, ᴛᴜ ɴɪᴠᴇʟ ᴀᴄᴛᴜᴀʟ ᴇs:* ${_user.level}`, contextInfo: {externalAdReply : {mediaUrl: null, mediaType: 1, description: null, "title": wm, body: '', previewType: 0, "thumbnail": img.getRandom(), sourceUrl: [nna, md, yt, nn, ig, tiktok].getRandom()}}}, { quoted: m })             
...
if (m.limit) m.reply(`*${+m.limit}* ᴅɪᴀᴍᴀɴᴛᴇ 💎 ᴜsᴀᴅᴏs`)
if (m.money) m.reply(+m.money + ' ᴄᴏɪɴs ᴜsᴀᴅᴏs') 
}
break
}}} catch (e) {
console.error(e)
...
if (!m.fromMem && m.text.match(/(@5492266466080|LoliBot|Botsito|Gata|ChinaMitzuki|china|mitzuki|chinaa|:v)/gi)) {
...
let or = ['texto']; // Solo dejamos la opción 'texto' para evitar audio
let media = or[Math.floor(Math.random() * 1)] // Solo se elige texto

// El mensaje de bienvenida que se enviará
let welcomeMessage = "¡Bienvenido! Estamos felices de tenerte aquí. Si necesitas ayuda, solo pregúntame."

if (media === 'texto') {
    this.sendMessage(id, {
        text: welcomeMessage,  // Se envía el mensaje de bienvenida
        contextInfo: {
            forwardingScore: 9999999,
            isForwarded: true,
            mentionedJid: [user]  // Solo la mención al usuario sin descripción ni ad
        }
    }, { quoted: fkontak2 })  // No se incluye ningún archivo adicional ni descripción
}
...
export async function callUpdate(callUpdate) {
    ...
    if (nk.isGroup == false) {
    if (nk.status == "offer") {
    let callmsg = await this.reply(nk.from, `ʜᴏʟᴀ *@${nk.from.split('@')[0]}*, ʟᴀs ${nk.isVideo ? 'videollamadas' : 'llamadas'} ɴᴏ ᴇsᴛᴀɴ ᴘᴇʀᴍɪᴛɪᴅᴀs, sᴇʀᴀs ʙʟᴏǫᴜᴇᴀᴅᴏ.\n\nsɪ ᴀᴄᴄɪᴅᴇɴᴛᴀʟᴍᴇɴᴛᴇ ʟʟᴀᴍᴀsᴛᴇ ᴘᴏɴɢᴀsᴇ ᴇɴ ᴄᴏɴᴛᴀᴄᴛᴏ ᴄᴏɴ ᴍɪ ᴄʀᴇᴀᴅᴏʀᴀ ᴘᴀʀᴀ ǫᴜᴇ ᴛᴇ ᴅᴇsʙʟᴏǫᴜᴇᴇ!\n\nhttps://www.instagram.com/mitzuki_chinita`, false, { mentions: [nk.from] })
    //let data = global.owner.filter(([id, isCreator]) => id && isCreator)
    //await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
    let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;𝕮𝖍𝖎𝖓𝖆 𝕸𝖎𝖙𝖟𝖚𝖐𝖎 👑;;;\nFN:𝕮𝖍𝖎𝖓𝖆 𝕸𝖎𝖙𝖟𝖚𝖐𝖎 💋\nORG:𝕮𝖍𝖎𝖓𝖆 𝕸𝖎𝖙𝖟𝖚𝖐𝖎 💋\nTITLE:\nitem1.TEL;waid=85254109507:+852 5410 9507\nitem1.X-ABLabel:𝕮𝖍𝖎𝖓𝖆 𝕸𝖎𝖙𝖟𝖚𝖐𝖎 💋\nX-WA-BIZ-DESCRIPTION:[❗] ᴇsᴄʀɪʙɪ sᴏʟᴏ ᴘᴏʀ ᴄᴏsᴀs ᴅᴇʟ ʙᴏᴛ.\nX-WA-BIZ-NAME:𝕮𝖍𝖎𝖓𝖆 𝕸𝖎𝖙𝖟𝖚𝖐𝖎 💋\nEND:VCARD`
    await this.sendMessage(nk.from, { contacts: { displayName: '𝕮𝖍𝖎𝖓𝖆 𝕸𝖎𝖙𝖟𝖚𝖐𝖎 💋', contacts: [{ vcard }] }}, {quoted: callmsg})
...
global.dfail = (type, m, conn, usedPrefix) => {
    let msg = {
        rowner: 'დ ¡Acceso restringido! Este comando es exclusivo para mi propietaria. ¡Lo siento! 🔒',
        owner: 'ღ ¡Comando reservado! Este comando es solo para mi propietaria. ¡Privilegios exclusivos! 🔒',
        mods: 'ღ Este comando solo lo puedo usar yo. ¡Privilegios de moderador! 😘',
        premium: 'ღ Este comando es solo para usuarios Premium (VIP). ¡Ser VIP tiene sus beneficios! 🌟',
        group: 'ღ ¡Comando para grupo! Este comando solo funciona en grupos.',
        private: 'ღ ¡Comando solo funciona el privado! Vamos al privado, este comando solo funciona en el privado del bot. ¡Hablemos en privado! 🤫',
        admin: 'ღ ¡Necesitas ser admins! Solo los admins pueden usar este comando. ¡Necesito a los jefes aquí! 🛡️',
        botAdmin: 'ღ ¡Necesito permisos! bb primero hazme admins al Bot osea a mi para poder usar este comando.',
        unreg: '「NO ESTAS REGISTRADO」\n\nMi amor no estás registrado en mi base de datos 💋\n\nPara poder usarme escribe el siguente comando\n\nComando: #reg nombre.edad\nEjemplo: #reg china.15',
        restrict: '[ 🔐 ] ¡Comando desactivado! Este comando esta desactivado por mi jefa'
    }[type]
